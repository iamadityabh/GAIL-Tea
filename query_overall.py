...
def handle_overall_query(user_question, current_mem_key):
    status_box = st.status("🛠️ Fetching Team & Department Data...", expanded=True)
    ...
    with status_box:
        ...
        try:
            ...
            st.write("🏢 **Departments Loaded from Cache**")
            ...
            if dynamic_results:
                st.write(f"🔍 Found {len(dynamic_results)} relevant employee records.")
                ...
                st.write("📄 **Employee Data Passed to LLM**")
            else:
                sql_json_data = "[]"
    ...
    status_box.update(label="✅ Data Processed", state="complete", expanded=False)
