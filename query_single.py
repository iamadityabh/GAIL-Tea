...
3. ABSOLUTE TRUTH: The STRUCTURED DATABASE RECORD (JSON) is the ONLY truth for basic info (phone_no, age, position, etc.). You must completely BLIND YOURSELF to the UNSTRUCTURED PDF DATA for these fields. 
4. NO AUTOCORRECT: Even if a value in the JSON looks fake, incomplete, or like a typo (e.g., '100' for a phone number), YOU MUST USE THE JSON VALUE EXACTLY. DO NOT autocorrect using the PDF data.
